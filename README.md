...
```js
const SERVER = "https://your-tunnel-or-api-url.example.com";
```
...
> ⚠️ **Note:** if you're exposing your device via a temporary tunnel (e.g. Cloudflare Tunnel's `trycloudflare.com`), that URL changes every time the tunnel restarts. Update `SERVER` accordingly, and avoid committing a live tunnel URL to a public repo since it grants direct network access to whatever is running behind it.
...
```bash
git clone https://github.com/dhruvpandit46/Test_2_Home.git
```
